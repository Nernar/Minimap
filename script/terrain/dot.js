...
const heightmapDot = [
	...
	function nearestOptimumSurface(ix, iz) {
		let iy = DIMENSION == 1 ? findSurface(ix, 80, iz) : findSurfaceUnderSky(ix, 256, iz);
		do {
	...
	function nearestSurfaceUnderSky(ix, iz) {
		return findSurfaceUnderSky(ix, 256, iz);
	}
];
...
const mapDot = [
	function monochromaticColormap(ix, iz) {
		...
		let meta = World.getBlockData(ix, iy, iz);
		if (block == 385) {
			block = 9;
		}
		let color = 0;
		if (biomeColormap.hasOwnProperty(block)) {
			let biome = World.getBiome(ix, iz);
	...
	function undergroundMap(ix, iz) {
		...
		let increment = 3;
		do {
			blockNew = World.getBlockID(ix, iy, iz);
